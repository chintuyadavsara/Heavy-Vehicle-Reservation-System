...
<?php
		...
		$Owner_id=$_SESSION['owner_id'];
			echo"<h1 class='jumbotron '><center>My Taking</center></h1>";
			...
			echo "<table class='table table-hover' >
				...
				<tbody>";
			$query1="select Picture from registration where Email='$Owner_id'";
			$rs1=mysql_query($query1);
			$row=mysql_fetch_array($rs1);
			$Image=$row[0];
			
			$query="select * from Booking where Status='Book' and OwnerEmail='$Owner_id'";
			$rs=mysql_query($query);
			
			while($row1=mysql_fetch_array($rs)){
					echo "
						<tr>
				    <td><img src='$Image' width=150px height=80px /></td>
				    <td><strong>  $row1[0] </strong></td>
				    <td><strong>  $row1[1] </strong></td>
				    <td><strong>  $row1[6] </strong></td>
				    <td><strong>  $row1[7] </strong></td>
				    <td><strong>  $row1[8] </strong></td>
				    <td><a href='Done_Booking.php?id='$Owner_id'><button class='btn btn-info' >Done</button></a></td>
				  </tr>"
					;
					;
		...
		echo"
		<center><h1 class='jumbotron'>Please Login to see your Booking</h1></center>
		";
	}
